...
void Pic_Mar_COSY_ThreeFirstOrderCorrectionJuly()
{
	...
	gUncorrectedJuly[0]->GetYaxis()->SetRangeUser(0,9.5);
	gUncorrectedJuly[1]->GetYaxis()->SetRangeUser(0,18);
	gUncorrectedJuly[2]->GetYaxis()->SetRangeUser(0,2.5);
	
	TLatex *ProblemText = new TLatex();
	ProblemText->SetTextSize(0.05);
	//ProblemText->SetTextAngle(-80);
	ProblemText->SetTextAlign(12);
	TArrow *ProblemArrow=new TArrow();
	ProblemArrow->SetLineWidth(2);
	ProblemArrow->SetLineColor(1);
	ProblemArrow->SetFillColor(1);
	ProblemArrow->SetArrowSize(0.02);
	ProblemArrow->SetOption("|>");
	for(int i =0;i<3;i++)
	{
		Can->cd();
		Pads[i]->Draw();
		...
		//gCorr1TDeriMax90July[i]->Draw("apl");
		gUncorrectedJuly[i]->Draw("apl");
		gUncorrectedJuly[i]->GetXaxis()->SetTitleOffset(1.35);
		gPad->Update();
		gPad->Modified();
		//ProblemArea->DrawBox(0.3,gPad->GetUymin(),1.3,gPad->GetUymax());
		gUncorrectedJuly[i]->Draw("pl");
		gCorr1T1090July[i]->Draw("pl");
		gCorr1T10DeriMaxJuly[i]->Draw("pl");
		gCorr1TDeriMax90July[i]->Draw("pl");
		if(i<2)
		{
			ProblemText->DrawLatexNDC(0.15,0.77,"#splitline{HV and DAQ}{crash}");
			if(i==0)
				ProblemArrow->DrawArrow(0.85,7.35,1.6,6.1);
			else
				ProblemArrow->DrawArrow(0.85,7.35*18/9.5,1.6,6.1*18/9.5);
		}
		else
		{
			ProblemText->DrawLatexNDC(0.25,0.85,"#splitline{HV and DAQ}{crash}");
			ProblemArrow->DrawArrow(1.3,2.2,1.6,1.9);
		}
	}
	
	Pads[0]->cd();
	//Can->cd(1);
		//gUncorrectedJuly[0]->GetYaxis()->SetRangeUser(0,10);
		...
		TLegend *leg=new TLegend(0.6,0.2,0.9,0.5);
			...
			leg->AddEntry(gCorr1T10DeriMaxJuly[0],"t_{10Imax}","p");
			leg->AddEntry(gCorr1T1090July[0],"t_{1090}","p");
			leg->AddEntry(gCorr1TDeriMax90July[0],"t_{Imax90}","p");
		leg->Draw();
		
	Pads[1]->cd();
	//Can->cd(2);
		//gUncorrectedJune[1]->GetYaxis()->SetRangeUser(0,35);
		//ProblemText->DrawLatexNDC(0.25,0.67,"#splitline{HV and DAQ}{crash}");
		gPad->Update();
		gPad->Modified();
	...
	gPad->Update();
		...
		GausText->DrawText(1.5,1.6,"FWTM/FWHM gaussian");
		gPad->Update();
		gPad->Modified();
		Can->SaveAs("Mar_COSY_ThreeFirstOrderCorrectionJuly.png");
		//if (1) andi::saveCanvas_allFileNames(Can, "~/pictures/Mar_COSY_ThreeFirstOrderCorrectionJuly" );
		if (SaveImages) andi::saveCanvas_allFileNames(Can, "~/pictures/Mar_COSY_ThreeFirstOrderCorrectionJuly" );
	return ;
}
